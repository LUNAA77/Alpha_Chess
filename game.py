...
class GameLoop(QThread):  # Thread responsible for chess piece movement of AIs
    ...
    # Signal for chess moving
    step_signal = pyqtSignal(int, int, int, int, tuple)
    ...
    def run(self):
        ...
        while True:
            ...
            action_history = (
                action[0], action[1], action[2], action[3], board[action[2]][action[3]])
            ...
            self.step_signal.emit(
                action[0], action[1], action[2], action[3], board)
