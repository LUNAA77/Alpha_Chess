...
# Used for synchronization delay during pop-up window to ensure execution sequence
class ShowMessageThread(QThread):
    ...
    def run(self):
        ...
        # Transmit signals for text content
        self.signal.emit(self.title, self.text)
...
class GameGUI(QWidget):   # Custom window class
    ...
    # Using the chessboard coordinate system, x is for vertical and y is for horizontal
    move_start_y = -1
    ...
    def window_init(self, window_name):  # Initialize the form
        ...
        self.move(int((screen.width() - size.width()) / 2),
                  int((screen.height() - size.height()) / 2))  # Center the window
        self.setWindowTitle(window_name)   # Set title
        palette = QPalette()    # Palette Object
        palette.setColor(QPalette.Background, QColor(
            35, 143, 130))   # Set palette colors
        self.setPalette(palette)
        self.setFixedSize(self.width(), self.height())  # Fix window size

    # Set chess piece movement markers
    def set_move_mark(self, flag_x=10, flag_y=10, pos_x=10, pos_y=10, des_x=10, des_y=10):

        # 10 means remain unchanged, -1 means not draw
        if flag_x != 10:
            self.select_x = flag_x
        if flag_y != 10:
            self.select_y = flag_y
        if pos_x != 10:
            self.move_start_x = pos_x
        if pos_y != 10:
            self.move_start_y = pos_y
        if des_x != 10:
            self.move_end_x = des_x
        if des_y != 10:
            self.move_end_y = des_y
        self.draw_flag = True
    ...
    def paintEvent(self, event):
        ...
        for i in range(9):
            # Draw column lines with a blank line in the middle
            self.board_draw_line(0, i, 4, i, painter)
            self.board_draw_line(5, i, 9, i, painter)

        # Fill in the blank lines on both sides
        self.board_draw_line(4, 0, 5, 0, painter)
        self.board_draw_line(4, 8, 5, 8, painter)

        # Draw the diagonal line of the Nine Palaces above the chessboard
        self.board_draw_line(0, 3, 2, 5, painter)
        self.board_draw_line(0, 5, 2, 3, painter)

        # Draw the diagonal line of the Nine Palaces below the chessboard
        self.board_draw_line(7, 3, 9, 5, painter)
        self.board_draw_line(7, 5, 9, 3, painter)

        # Draw the "楚河汉界"
        painter.setFont(QFont("Timers", 28, QFont.Bold))
        painter.drawText(self.start_x + 2 * self.chess_x,
                         int(self.start_y + 5.2 * self.chess_y), "楚河         汉界")
        ...
        for i in range(9):
            t = i
            # Vertical coordinates marked above the chessboard
            up_y = int(self.start_y - self.chess_y / 2)
            # Vertical coordinates marked below the chessboard
            down_y = int(self.start_y - self.chess_y / 2 + 10.6 * self.chess_y)
            ...
            # Horizontal coordinates marked above the chessboard
            up_x = int(self.start_x + self.chess_x / 2 + t * self.chess_x) - 5
        ...
        if self.move_start_x != -1:
            ...
            if self.board_reverse:  # Need to flip view
                x = 8 - x
                y = 9 - y
            painter.drawRect(self.start_x + self.chess_x * x,
                             self.start_y + self.chess_y * y, self.chess_x, self.chess_y)
    ...
    # Draw lines using a chessboard array as coordinate points
    def board_draw_line(self, begin_x, begin_y, end_x, end_y, painter):

        painter.drawLine(int(self.start_x + self.chess_y / 2 + begin_y * self.chess_y),
                         int(self.start_y + self.chess_x /
                             2 + begin_x * self.chess_x),
                         int(self.start_x + self.chess_y /
                             2 + end_y * self.chess_y),
    ...
    def GameShow(self, board):  # Redraw interface
        ...
        for i in range(10):
            for j in range(9):
                if board[i][j] != 0:
                    # Get the corresponding name of the chess piece
                    name = id_map.get(board[i][j])
                    ...
                    if (self.board_reverse):    # Flip the view if your side is black
                        label.move(self.start_x + (8 - j) * self.chess_x,
                                   self.start_y + (9 - i) * self.chess_y)
                    else:
                        label.move(self.start_x + j * self.chess_x,
                                   self.start_y + i * self.chess_y)

                    label.resize(self.chess_x, self.chess_y)  # Set label size

                    # Get image location
                    # Get relative path, omit qrc file
                    ...
                    # Load images and set image size
                    label.setPixmap(QPixmap(path).scaled(
                        self.chess_x, self.chess_y))
                    label.show()    # Output label content and display chess pieces
                    # Add tags to the tag array for recording and refreshing the interface
                    self.chess.append(label)
