...
class PVELoop(QThread):  # Thread responsible for chess piece movement of AI
    ...
    def run(self):
        ...
        if len(action_list) == 0:
            ...
            history.append(
                (action[0], action[1], action[2], action[3], eaten_id))
...
class PVE(GameGUI):  # Inherit GameGUI
    ...
    # side is yours, if side is "black", then you take the black side
    def __init__(self, side="red", player=None):
        ...
        self.game_thread.step_signal.connect(
            self.convey)  # Thread binding slot function
    ...
    def mousePressEvent(self, event):
        ...
        if event.buttons() == QtCore.Qt.LeftButton and self.victory == None:    # Press the left button
            ...
            x, y = (y - self.start_y) // self.chess_y, (x -
                                                        self.start_x) // self.chess_x
            if self.side == "black":   # Flip view
                x = 9 - x
                y = 8 - y
            # Pass array coordinates to game
            draw_flag, move_flag = self.step(x, y)
            if draw_flag or move_flag:
                # If any piece move, redraw the chessboard
                self.GameShow(board)
            if move_flag:
    ...
    def step(self, x, y):  # One step for your action
        ...
        if self.round == self.side:   # When you are playing chess
            if self.chosen == False:   # When no chess piece have been selected yet
                ...
                # Change the selected state of the chess piece to unselected state if click elsewhere
                self.chosen = False
                self.set_move_mark(-1, -1)
                if 0 <= x <= 9 and 0 <= y <= 8:    # Check if the chessboard array is out of bounds
                    action_list = get_legal_actions(board, self.side, history)
                    # When complying with the chess piece movement rules
                    if (self.old_x, self.old_y, x, y) in action_list:
                        self.set_move_mark(-1, -1, self.old_x,
                                           self.old_y, x, y)
                        history.append(
                            (self.old_x, self.old_y, x, y, board[x][y]))
